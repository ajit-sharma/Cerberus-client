...
class ClassQuery extends QueryHandler {
    ...
    protected RefObject printReferencesTo(JavaHeapObject obj) {    	
        if (obj.getId() == -1) {
            return null;
        }
        ...
        //System.out.println(referers.hasMoreElements() + " " + obj.getSize() + " " +obj.getClazz().getName());
                
        RefObject refobj = new RefObject();
        refobj.setObjectChain(obj.getClazz().getName(),obj.getSize(), obj.getIdString());
        while (referers.hasMoreElements()) {
            JavaHeapObject ref = (JavaHeapObject) referers.nextElement();
            printThing(ref);

           // instance_list.add(new RefObject(ref.getClazz().getName(), ref.getSize()));
            
            
//        	refobj.setObjectChain(ref.getClazz().getName(),ref.getSize(), ref.getIdString());

            if( !reference_list_id_chk.contains(ref.getIdString()) && reference_list_id.size() < 15 )
            {
            	reference_list_id.add(ref.getIdString());    	
            	reference_list_id_chk.add(ref.getIdString());    
            	refobj.addRef(printReferencesTo(ref));            	    
            }
            else
            {  
            	reference_list_id.remove(obj.getIdString()); 
            	return refobj;          
            }
            //refobj = new RefObject();
            //System.out.println(ref.getSize() + " " +ref.getClazz().getName());            
        ...
        //System.out.println(reference_list.toString());
