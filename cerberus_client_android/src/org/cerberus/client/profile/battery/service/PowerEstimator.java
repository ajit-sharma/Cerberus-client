...
import org.cerberus.client.profile.battery.component.OLED;
import org.cerberus.client.profile.battery.component.PowerComponent;
import org.cerberus.client.profile.battery.phone.PhoneConstants;
import org.cerberus.client.profile.battery.phone.PhoneSelector;
import org.cerberus.client.profile.battery.phone.PowerFunction;
import org.cerberus.client.profile.battery.util.BatteryStats;
import org.cerberus.client.profile.battery.util.Counter;
import org.cerberus.client.profile.battery.util.HistoryBuffer;
...
public class PowerEstimator implements Runnable {
  ...
  private void openLog(boolean init) {
	  Log.i("openLog", "calledd");
    /* Open up the log file if possible. */
    try {
    	String logFilename = 
    			Environment.getExternalStorageDirectory() + "/" + "PowerTrace.log";
      Log.i("openLog", logFilename);
  ...
  public void run() {
    ...
    for(long iter = -1; !Thread.interrupted(); ) {
    	...
    	Log.i("PowerEstimator run", "Thread.interrupted - " + Thread.interrupted());
...
//            }
    ...
    Log.i("PowerEstimator", "finish run " + Thread.interrupted());
...
//    PowerWidget.updateWidgetDone(context);
    ...
    synchronized(fileWriteLock) {
      if(logStream != null) try {
    	  Log.i("FileWriteLock", "-----");
        logStream.close();
        PowerEstimatorCheck.updateState(PowerEstimatorCheck.DIE);
      } catch(IOException e) {
